...
Push_loc(void)
{
    Counter_level++;
    Counter++;
    if( Last_data_ref < Counter_level ) {
        *Counter = 0;
        Last_data_ref++;
        Put_load_ref('t', Counter_level, Create_name(Counter_level));
    }
    Flush_load_bufs();
}
...
{
    Flush_load_bufs();
    Rload_word( PTR_RELDESC, Counter_level+1, Counter[1] );
}
