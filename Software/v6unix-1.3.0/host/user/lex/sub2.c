...
void first(v)	/* calculate set of positions with v as root which can be active initially */
  int v; {
	register int i;
	register char *p;
	i = name[v];
...
void packtrans(st,tch,tst,cnt,tryit)
...
# endif
void acompute(s)	/* compute action list = set of poss. actions */
  int s; {
	register int *p, i, j;
	int cnt, m;
...
# endif
	...
	if(optim){
		...
		if (chset==0) /* no chset, put out in normal order */
			...
			if (fbarr==0)
				error("No space for char table reverse");
			for(i=0; i<ZCH; i++)
				fbarr[i]=0;
			for(i=0; i<NCH; i++)
