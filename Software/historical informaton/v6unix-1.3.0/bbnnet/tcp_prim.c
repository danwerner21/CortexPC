...
struct mbuf *dat;
{
	...
	{
		m->m_len += TCP_M_LEN;
		len += TCP_M_LEN - 1;	/* SYN occupies seq space */
		t->t_off = (TCPSIZE + TCP_M_LEN) >> 2;
...
sequence start, end;
{
	...
	do {		/* main copy loop */
		...
		memcpy ((caddr_t)((int)n + MHEAD + n->m_len),
			(caddr_t)((int)m + m->m_off),
			len);
		n->m_len += len;
		off += len;
...
partial:		n->m_off = MHEAD;
			n->m_next = NULL;
			n->m_len = m->m_len - adj;
			memcpy((caddr_t)((int)n + MHEAD),
				(caddr_t)((int)m + m->m_off + adj),
				n->m_len);
			off += n->m_len;
		}
