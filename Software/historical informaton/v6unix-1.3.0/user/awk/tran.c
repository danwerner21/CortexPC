...
syminit()
{
	setsymtab("0", tostring("0"), fzero, NUM|STR|CON|FLD, symtab);
	recloc = setsymtab("$record", record, fzero, STR|FLD, symtab);
	dprintf("recloc %o lookup %o\n", recloc, lookup("$record", symtab), NULL);
	FS = &setsymtab("FS", tostring(" "), fzero, STR|FLD, symtab)->sval;
	RS = &setsymtab("RS", tostring("\n"), fzero, STR|FLD, symtab)->sval;
	OFS = &setsymtab("OFS", tostring(" "), fzero, STR|FLD, symtab)->sval;
	ORS = &setsymtab("ORS", tostring("\n"), fzero, STR|FLD, symtab)->sval;
	OFMT = &setsymtab("OFMT", tostring("%.6g"), fzero, STR|FLD, symtab)->sval;
	FILENAME = &setsymtab("FILENAME", NULL, fzero, STR|FLD, symtab)->sval;
	nfloc = setsymtab("NF", NULL, fzero, NUM, symtab);
	NF = &nfloc->fval;
	nrloc = setsymtab("NR", NULL, fzero, NUM, symtab);
	NR = &nrloc->fval;
}
...
#else
			vp->fval = atoi(vp->sval);
#endif
			if (!(vp->tval & CON))	/* don't change type of a constant */
				vp->tval |= NUM;
		}
		else
			vp->fval = fzero;	/* not a very good idea */
	}
	dprintf("  %g\n", vp->fval, NULL, NULL);
	return(vp->fval);
}
