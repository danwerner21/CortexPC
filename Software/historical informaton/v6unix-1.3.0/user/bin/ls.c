...
typedef struct { int filler; unsigned char dminor, dmajor;} DEV;
...
struct lbuf *ap;
{
	...
	if (lflg) {
		...
		if (p->ltype=='b' || p->ltype=='c') {
			d = (DEV *)&p->lsize;
			printf("%3d,%3d", d->dmajor, d->dminor);
		}
		else
			printf("%7ld", p->lsize);
		cp = ctime(&p->lmtime);
		...
		if (++count < maxcount) {
			if (p->lflags&ISARG)
				printf("%-15.15s", p->ln.namep);
			else
				printf("%-15.15s", p->ln.lname);
			}
		else {
			if (p->lflags&ISARG)
				printf("%s\n", p->ln.namep);
