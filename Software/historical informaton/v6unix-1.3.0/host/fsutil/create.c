	...
	const char *filename2)
{
	...
	/* Check a.out header. */
	if (read (fd, buf, 16) != 16 || ! (buf[1] == 7 && buf[0] == 1)) {
failed:		close (fd);
		close (fd2);
		return 0;
	}
	if (read (fd2, buf2, 16) != 16 || ! (buf2[1] == 7 && buf2[0] == 1))
		goto failed;

	/* Check .text+.data segment size. */
	n = buf[3] + (buf[2] << 8) + buf[5] + (buf[4] << 8);
	n2 = buf2[3] + (buf2[2] << 8) + buf2[5] + (buf2[4] << 8);
...
static int create_root_directory (u6fs_t *fs)
{
	...
	buf[1] = inode.number;
	buf[0] = inode.number >> 8;
	buf[2] = '.';
	buf[17] = inode.number;
	buf[16] = inode.number >> 8;
	buf[18] = '.';
	buf[19] = '.';
	inode.nlink = 2;
	inode.size = 32;
