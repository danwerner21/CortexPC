			...
			if(prev == '\n'){		/* next char is at beginning of line */
				...
				case '%':
					...
					case '%':
						...
						left = (long *)myalloc(treesize,sizeof(*left));
						right = (long *)myalloc(treesize,sizeof(*right));
...
case 7:
# line 50 "parser.y"
{	scopy((char *)yypvt[-1],dp);
		def[dptr] = dp;
		dp += slength((char *)yypvt[-1]) + 1;
		scopy((char *)yypvt[-0],dp);
		subs[dptr++] = dp;
		if(dptr >= DEFSIZE)
			error("Too many definitions");
		dp += slength((char *)yypvt[-0]) + 1;
		if(dp >= dchar+DEFCHAR)
